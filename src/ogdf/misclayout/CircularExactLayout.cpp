...
ILOHEURISTICCALLBACK2(heuristicCallback, IloBoolVarArray&, var, ogdf::CircularExactLayout *, cel) {
	...
	//compute the added xijValues for each node i - this is a sum of n-1 different values, should the value xij not exist, take (1 - xji)
...
namespace ogdf {
	...
	void CircularExactLayout::computeBicomp(node bcTreeNode)
	{
		...
		// "Now starting to compute a new biconnected Component!" <<std::endl ;
		/************************ prepare ILP **************************
		 * the approach to the ILP is as follows:
		 * only edges in the same biconnected component will ever cross,
		 * as in the resulting layout all nodes belonging to one bi-
		 * connected component will be placed in a block-cutpoint tree
		 * without producing additional crossings
		 */
	...
	void CircularExactLayout::writeLayout()
	{
		...
		double circleFactor = 2* Math::pi / m_G.numberOfNodes();
		for (int i = 0; i < m_G.numberOfNodes(); i++)
		{
			...
			GA.label(theNode) = to_string(m_knotenindex[theNode]);
		}
		string theFile = m_theFile;
		writeGML(GA, m_theFile);
	}
	...
	int CircularExactLayout::doGreedyInsertionSwitch(GraphAttributes &GA)
	{
		...
		double circleFactor = 2* Math::pi / G.numberOfNodes();
		...
		for (it = nodeOrder.begin(); it.valid(); ++it)
		{
			...
			GAGS.label(theNode) = to_string(knotenindex[theNode]);
			z++;
		}
		string theFile = "GreedySwitch.gml" ;
		GraphIO::writeGML(GAGS, theFile);

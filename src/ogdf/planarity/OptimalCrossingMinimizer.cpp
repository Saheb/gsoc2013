...
CrossingMinimizationModule *OptimalCrossingMinimizer::clone() const
{
	OptimalCrossingMinimizer *crossMin = new OptimalCrossingMinimizer;
...
int OptimalCrossingMinimizer::Subproblem::generateBranchRules(ArrayBuffer<BranchRule*> &rules) {
...
int OptimalCrossingMinimizer::Subproblem::checkSimplicitiesAndPrice() {
...
void OptimalCrossingMinimizer::Subproblem::deactivateNonLiftableConstraints(ArrayBuffer<Variable*>* buf) {
	int remd = 0;
	for(int i = nCon(); i-->0;) {
		bool reset = false;
...
//	#endif //OGDF_DEBUG
	...
	switch(error) {
		...
		default:
			lout() << "Unknown error happened in Abacus::solveLp (" << error << ")\n";
			return error;
	}
//	double lpRes=0;
//	for(int i = 0; i < nVar(); i++) {
...
void OptimalCrossingMinimizer::Master::doWriteBestSolution() {
	...
	if(writeResult()) {
		...
		GraphAttributes GAR(RR, GraphAttributes::nodeGraphics | GraphAttributes::nodeStyle);
		node n;
		forall_nodes(n, RR) {
...
void OptimalCrossingMinimizer::Master::hintsKnKuratowskiMinusOne(List<Constraint*>& prelist) {
	node n;
	edge e;
...
void OptimalCrossingMinimizer::Master::helperHintsKnAllSubKuratowskis(NodeArray<bool>& aktnodes, node posNode, int num, List<Constraint*>& prelist) {
	//node n;
	edge e;
	if(posNode) {
...
void OptimalCrossingMinimizer::Master::hintsKnAllSubKuratowskis(List<Constraint*>& prelist) {
...
void OptimalCrossingMinimizer::Master::hintsKnNodeOrder(List<Constraint*>& prelist) {
	node n;
...
void OptimalCrossingMinimizer::Master::hintsKnEdgeOrder(List<Constraint*>& prelist) {
	edge e;
...
void OptimalCrossingMinimizer::Master::hintsKnmKuratowskiMinusOne(List<Constraint*>& prelist) {
	node n;
	edge e;
...
void OptimalCrossingMinimizer::Master::hintsKnExpensiveKuratowski(List<Constraint*>& prelist) {
	node n,nn;
	edge e;
...
void OptimalCrossingMinimizer::Master::hintsKnmAllSubKuratowskis(List<Constraint*>& prelist) {
	lout(LL_FORCE) << "WARNING: KnmAllSubKuratowskis not implemented! -> no such hints generated!!!";
	OGDF_ASSERT(false);
}

void OptimalCrossingMinimizer::Master::hintsKnmNodeOrder(List<Constraint*>& prelist) {
	node n;
	edge e;
...
void OptimalCrossingMinimizer::Master::hintsKnmEdgeOrder(List<Constraint*>& prelist)
{
	node baseNode = minimizedGraph->firstNode();
...
int OptimalCrossingMinimizer::Master::enumerationStrategy(const Sub* s1, const Sub* s2) {

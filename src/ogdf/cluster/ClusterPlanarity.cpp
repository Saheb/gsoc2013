...
bool ClusterPlanarity::isClusterPlanar(const ClusterGraph &CG, List<nodePair> &addedEdges) {
	...
	if (m_solmeth==sm_new)
	{
		...
		for (int i = 0; i < numIndyBags; i++)
		{
			...
			ListConstIterator<node> it = nodesInBag[i].begin();
			while (it.valid())
			{
				node u = (*it);
			...
			while (!queue.empty())
			{
				...
				{
					ListConstIterator<node> itn = cNodes[c].begin();
					while (itn.valid())
					{
						bagCG.reassignNode(*itn, cCopy[c]);
						itn++;
					}
				}
				ListConstIterator<cluster> itc = cChildren[c].begin();
				while (itc.valid())
				{
					cCopy[*itc] = bagCG.newCluster(cCopy[c]);;
					queue.pushBack(*itc);
			...
			while (!cqueue.empty())
			{
				...
				ListConstIterator<cluster> itc =  t->cBegin();
				while (itc.valid())
				{
					cqueue.pushBack(*itc);
					itc++;
			...
			if (theGraphs[i]->numberOfNodes() > 2) //could even start at 4...
			{
				...
				ListIterator<nodePair> itnp = ae.begin();
				while (itnp.valid())
				{
					addedEdges.pushBack(nodePair(theGraphs[i]->original((*itnp).v1),theGraphs[i]->original((*itnp).v2)));
					itnp++;
				}
			}
#ifdef OGDF_DEBUG
		...
		ListIterator<nodePair> it = ae.begin();
		while (it.valid())
		{
			addedEdges.pushBack(nodePair(nodeOrig[(*it).v1],nodeOrig[(*it).v2]));
			it++;
		}
...
//	if (m_solmeth==sm_new) return doFastTest(G,addedEdges);
	...
	cplanMaster = new CPlanarityMaster(G,
									 ...
									 m_kuratowskiLow,
									 m_perturbation);
	if (m_solmeth==sm_new) ((CPlanarityMaster*)cplanMaster)->setSearchSpaceShrinking(true);
	else ((CPlanarityMaster*)cplanMaster)->setSearchSpaceShrinking(false);

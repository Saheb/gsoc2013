...
namespace ogdf {
	...
	// Simple debug output of the whole graph. KK Is there any reason for having this method in release mode?
	...
	void KandinskyLayout::call(PlanRepUML &PG, adjEntry adjExternal, Layout &drawing)
	{
...
#ifndef OGDF_DEBUG
		//osi->setSymParam(OsiSymVerbosity, -2); // -2: no output (ausser bei Fehlern?)
#endif
		...
		{
			// remove added dummy vertices again
		...
		forall_nodes(n, intermediate) {
			node back;
			if ((back=nodeMappingBack[n]) != 0 && PG.original(back) != 0) {
				intermediateAttr.width(n) = PG.widthOrig(PG.original(back));
				intermediateAttr.height(n) = PG.heightOrig(PG.original(back));
			} else {
		...
		GraphIO::writeGML(extension, "layout1.gml");
#endif
		...
		forall_edges(e, PG)
		{
			...
			DPoint pOffset(0, 0);
			if (linedirS == 0) // N
				pOffset.m_x = offsetS * PG.widthOrig(PG.original(e->source()));
			else if (linedirS == 1) // E
				pOffset.m_y = offsetS * PG.heightOrig(PG.original(e->source()));
			...
			DPoint pOffsetT(0, 0);
			if (linedirT == 0)
				pOffsetT.m_x = offsetT * PG.widthOrig(PG.original(e->target()));
			else if (linedirT == 1)
				pOffsetT.m_y = offsetT * PG.heightOrig(PG.original(e->target()));
			else if (linedirT == 2)
				pOffsetT.m_x = -offsetT * PG.widthOrig(PG.original(e->target()));
			else if (linedirT == 3)
				pOffsetT.m_y = -offsetT * PG.heightOrig(PG.original(e->target()));
		...
		AdjEntryArray<double> &aeOffsets, AdjEntryArray<int> &aeNumNeighbours)
	{
		...
		forall_nodes(n, emb.getGraph())
		{
			...
			forall_adj(ae, n)
			{
				cout << ++i << ". ae " << setfill('0') << setw(2) << ae->index() << " \t edge "
					<< setw(2) << ae->theEdge()->index() << " ("
					<< setw(2) << ae->theNode()->index() << "->"
					<< setw(2) << ae->twinNode()->index()
					<< ")\t angle " << getAngleFor(ae, sol, ncols)
					<< ", bend " << getBendFor(ae, sol, ncols)
					<< ", edgebends " << getBendString(ae, sol, ncols).toString() << "\n";
			}
#endif

			adjEntry historicAdj = NULL, insertAdj = NULL;
			do
			{
				...
				while (!left.empty() || !right.empty() || !straight.empty())
				{
					...
					if (curnode == 0)
					{
						...
						{
#ifdef OGDF_DEBUG
							if (!insertAdj || insertAdj->theNode() != prednode)
	...
	{
		BendString rev;
		for (size_t i = 0; i < bs.size(); ++i)   //== warum nicht einfacht rueckwaerts durchgehen
		{
			rev += (bs[i] == '1' ? "0" : "1");
		}
		return rev;
	...
	BendString KandinskyLayout::getBendString(adjEntry ae, const double * sol, const int E)
	{
		...
		int bends = static_cast<int>(sol[base]), bendsrev = static_cast<int>(sol[base + 1]); //KK added cast to avoid warnings, but did not check overall correctness of the operation

...
/** \file
 * \brief Declaration of class OptimalSimultaneousCrossingMinimizer
 *
...
#ifndef OGDF_ABACUS_OPTIMAL_SIMULTANEOUS_CROSSING_MINIMIZER_SUPER_PRICE_H
#define OGDF_ABACUS_OPTIMAL_SIMULTANEOUS_CROSSING_MINIMIZER_SUPER_PRICE_H
...
class OGDF_EXPORT OptimalSimultaneousCrossingMinimizer : public CrossingMinimizationModule {
...
protected:
	...
	class CrossingVariable : public CrossingLocation, public abacus::Variable {
		friend std::ostream &operator<<(std::ostream &os, const OptimalSimultaneousCrossingMinimizer::CrossingVariable& v);
	public:
		CrossingVariable(abacus::Master* m, const Segment& s1, const Segment& s2) :
				CrossingLocation(s1,s2),
				abacus::Variable(m,0,0,0, // master / sub=0 / dynamic=false / local=false
					((OptimalSimultaneousCrossingMinimizer::Master*)m)->getCost(s1.e, s2.e)+
						((OptimalSimultaneousCrossingMinimizer::Master*)m)->epsilonify(s1, s2), // obj
					0,1,abacus::VarType::Binary) {} // lowerBound / upperBound / type
		bool correspondsTo(const Segment& s) const {
			return (s1 == s) || (s2 == s);
		}
		bool correspondsTo(edge e, int num) const {
			return (e == s1.e && s1.seg == num)|| (e == s2.e && s2.seg == num);
		}
		bool correspondsTo(edge e) const {
			return e == s1.e || e == s2.e;
		}
		bool correspondsTo(node n) const {
	...
	protected:
		inline OptimalSimultaneousCrossingMinimizer::Master* master() const {
			return (OptimalSimultaneousCrossingMinimizer::Master*)master_;
		}
		OGDF_NEW_DELETE
	};

	class SimplicityConstraint : public AbacusConstraint {
		friend std::ostream &operator<<(std::ostream &os, /*const*/ OptimalSimultaneousCrossingMinimizer::SimplicityConstraint& k);
		Segment s;
	public:
		SimplicityConstraint(abacus::Master* m, const Segment& t) :
		...
		double coeff(const abacus::Variable* cvar) const {
			return (double) ((const CrossingVariable*)cvar)->correspondsTo(s);
		}
	...
	class SegmentOrderConstraint : public AbacusConstraint {
		Segment s;
	public:
		SegmentOrderConstraint(abacus::Master* m, Segment t) :
			AbacusConstraint(m, 0, abacus::CSense::Greater, 0, 0, 0, 1),  // non-dynamic, non-local, pseudo-liftable
			s(t) {}
		double coeff(const abacus::Variable* cvar) const {
			const CrossingVariable& v = *(const CrossingVariable*)cvar;
			int pred = s.seg-1;
			//OGDF_ASSERT( pred != 0 );
	...
	class FirstSegmentConstraint : public AbacusConstraint {
		...
		int length;
	public:
		FirstSegmentConstraint(abacus::Master* m, edge t) :
				AbacusConstraint(m, 0, abacus::CSense::Greater, 0, 0, 0, 1),  // non-dynamic, non-local, pseudo-liftable
//				AbacusConstraint(m, 0, abacus::CSense::Greater, -1, 0, 0, 1),  // non-dynamic, non-local, pseudo-liftable
				e(t) {
		...
		edge referenceEdge() const { return e; }
		virtual const char* name() const {
	...
	class EdgeOrderConstraint : public AbacusConstraint {
		edge e1, e2;
	public:
		EdgeOrderConstraint(abacus::Master* m, edge te1, edge te2) :
			AbacusConstraint(m, 0, abacus::CSense::Greater, 0, 0, 0, 1),  // non-dynamic, non-local, pseudo-liftable
			e1(te1), e2(te2) {}
		double coeff(const abacus::Variable* cvar) const {
			return ((const CrossingVariable*)cvar)->correspondsTo(e1) - ((const CrossingVariable*)cvar)->correspondsTo(e2);
		}
		virtual const char* name() const {
	...
	class NodeOrderConstraint : public AbacusConstraint {
		node n1, n2;
	public:
		NodeOrderConstraint(abacus::Master* m, node tn1, node tn2) :
			AbacusConstraint(m, 0, abacus::CSense::Greater, 0, 0, 0, 1),  // non-dynamic, non-local, pseudo-liftable
			n1(tn1), n2(tn2) {}
		double coeff(const abacus::Variable* cvar) const {
			return ((const CrossingVariable*)cvar)->correspondsTo(n1) - ((const CrossingVariable*)cvar)->correspondsTo(n2);
		}
		virtual const char* name() const {
	...
	public:
		...
		static int typeToCr(KuratowskiType kt) {
			...
			return bipartiteZara(kt >> 16, kt & 0xffff);
		}
	...
	private:
		struct SegmentRange {
			...
			bool equals(const SegmentRange& ei) const {
	...
	public:
		...
		friend std::ostream &operator<<(std::ostream &os, /*const*/ OptimalSimultaneousCrossingMinimizer::KuratowskiConstraint& k);
		friend class Subproblem;

		KuratowskiConstraint(abacus::Master* m, const Subproblem* S, const GraphReduction& R, const KuratowskiSubdivision& K, bool dynamic, int rness = 0, bool local = false) :
		...
		KuratowskiConstraint(const KuratowskiConstraint& orig) : AbacusConstraint(orig), Logger(), edges(), D(orig.D), type(orig.type) {
//			OGDF_ASSERT( this->liftable() );
		...
		void branchMe(const Subproblem* S, KuratowskiConstraint*& k1, KuratowskiConstraint*& k2) {
			...
			OGDF_ASSERT( this->sense()->sense() ==  abacus::CSense::Greater );
		...
		KuratowskiConstraint(abacus::Master* m, KuratowskiType ltype, bool dynamic, int rness = 0, bool local = false) :
				AbacusConstraint(m, 0, RSense(rness),
		...
		virtual bool equal(const abacus::ConVar *cv) const {
			return equals(*(const KuratowskiConstraint*)cv);
		}
		...
		double coeff(const abacus::Variable* cvar) const {
			// we have to cast-away const from cvar as the array D stores non-const pointers!
			 CrossingLocationComparer::CrossingLocationPointer v = (CrossingLocationComparer::CrossingLocationPointer)const_cast<abacus::Variable*>(cvar);
	...
	public:
		...
		int checkSimplicitiesAndPrice();
		void deactivateNonLiftableConstraints(/*const*/ ArrayBuffer<abacus::Variable*>* vars);
	...
	protected:
		...
		ArrayBuffer< abacus::PoolSlot<abacus::Constraint, abacus::Variable>* > deactNonLiftCons;
		...
		OptimalSimultaneousCrossingMinimizer::Master* master() const {
			return (OptimalSimultaneousCrossingMinimizer::Master*)master_;
		}

		void findKuratowski(Graph& R, KuratowskiSubdivision& K);
		OptimalSimultaneousCrossingMinimizer::CrossingConfiguration* callBoundHeuristic(bool integer);
		inline double generateVariable(edge e1, int e1id, edge e2, int e2id, OptimalSimultaneousCrossingMinimizer::CrossingVariable** var);
	};

	friend std::ostream &operator<<(std::ostream &os, const OptimalSimultaneousCrossingMinimizer::CrossingLocation& v);
	friend std::ostream &operator<<(std::ostream &os, const OptimalSimultaneousCrossingMinimizer::CrossingVariable& v);
	friend std::ostream &operator<<(std::ostream &os, /*const*/ OptimalSimultaneousCrossingMinimizer::KuratowskiConstraint& k);
	friend std::ostream &operator<<(std::ostream &os, /*const*/ OptimalSimultaneousCrossingMinimizer::SimplicityConstraint& k);
	...
	class Master : /*public CrossingMinimizationModule,*/ public abacus::Master, protected Logger {
		...
		class ActiveVariables {
			OptimalSimultaneousCrossingMinimizer::Master* master;
			EdgeArray<int> perEdge;
			EdgeArray< EdgeArray<int* > > perSegmentAndEdge;

		public:
			ActiveVariables(OptimalSimultaneousCrossingMinimizer::Master* m) : master(m),
			...
			int numIndices(edge e)                   { return perEdge[e]; }
//			int numIndices(edge e, edge e2)          { return perEdgePair[e][e2].num(); }
			int numIndices(const Segment& s1, edge e2) { return perSegmentAndEdge[s1.e][e2][s1.seg]; }
//			int numIndices(edge e, int seg, edge e2) { return perEdgeSegmentAndEdge[e][e2][seg].num(); }

			bool isNew(edge e, int seg)              { return seg == perEdge[e]; }
			int numFree(edge e)                      { return master->expandedGraph->chain(e).size() - perEdge[e]; }
...
//				#endif
		...
		friend class OptimalSimultaneousCrossingMinimizer;
	...
	public:
		...
		Master() :
				abacus::Master("OptimalSimultaneousCrossingMinimizer::Master",1, true ,abacus::OptSense::Min),
	...
	//			lastRoundedCrossings(0),
		...
		abacus::Sub* firstSub() {
			return new OptimalSimultaneousCrossingMinimizer::Subproblem(this, 1000, numExpMaxCrossingPairs, upperbound, false);
		}
...
*/
	...
	private:
		...
		OptimalSimultaneousCrossingMinimizer::CrossingConfiguration* bestSolution;
		...
		void updateBestSolution(OptimalSimultaneousCrossingMinimizer::CrossingConfiguration* ncc) {
			if(bestSolution == ncc) return; // repetition -> init
			upperBoundSource = OptimalSimultaneousCrossingMinimizer::Master::SS_ILP_Heuristic;
		...
		int getCost(edge e1, edge e2) { // call with e1, e2 of minimizedGraph
			if(useSubgraphs()) {
				__uint32 n = subgraphs[e1]&subgraphs[e2];
				register __uint32 tmp;				    // Counting by MIT & X11: http://infolab.stanford.edu/~manku/bitcount/bitcount.html
				tmp = n - ((n >> 1) & 033333333333) - ((n >> 2) & 011111111111);
				return (((tmp + (tmp >> 3)) & 030707070707) % 63) * (useCost() ? cost[e1]*cost[e2] : 1);
			}
			return useCost() ? cost[e1]*cost[e2] : 1;
		}
		double epsilonify(const Segment& s1, const Segment& s2) {
			// espilonify++
			if(useSubgraphs() && !(subgraphs[s1.e]&subgraphs[s2.e]) ) {
				return 3*SEG_EPS - ( (s1.seg==0 && s2.seg==0) ? 0 : -SEG_EPS-(SEG_EPS*(1-(2*s1.seg+3*s2.seg)/double(5*(expansionFactor-1)))) );
			}
			return (s1.seg==0 && s2.seg==0) ? 0 : -SEG_EPS-(SEG_EPS*(1-(2*s1.seg+3*s2.seg)/double(5*(expansionFactor-1)))); // 0/0 are a *bit* more expensive...
			// simple espilonify
//			return (i1==0 && i2==0) ? 0 : -SEG_EPS; // 0/0 are a *bit* more expensive...
		}
		bool variableAllowed(edge e1, edge e2);

		abacus::StandardPool<abacus::Constraint, abacus::Variable>* simplicityPool;
		abacus::StandardPool<abacus::Constraint, abacus::Variable>* hintedPool;
		abacus::NonDuplPool<abacus::Constraint, abacus::Variable>* kuratowskiPool;
...
//		List<edge> helperEdgelist2;
		...
		void hintsKnEdgeOrder(List<abacus::Constraint*>& prelist);
...
/*		int m_numStartKuratowskis;
		...
		//! Reminder: OptimalSimultaneousCrossingMinimizerBase assumes that the given graph is connected
		OptimalSimultaneousCrossingMinimizer::CrossingConfiguration* helpCall(PlanRep &_PG, int cc, const EdgeArray<int>* _cost, const EdgeArray<bool> * _forbid, const EdgeArray<__uint32> * _subgraphs, int& crossingNumber);
		void generateMinimizedGraphCostForbidAndSubgraphs(const EdgeArray<int>  *_cost, const EdgeArray<bool> *_forbid, const EdgeArray<__uint32> *_subgraphs);
		void generateExpandedGraph();
		OptimalSimultaneousCrossingMinimizer::CrossingConfiguration* createHeuristicStartSolution();
...
protected:
	...
	virtual ReturnType doCall(PlanRep &PG,
		...
		blaster.m_useSubgraphs = (subgraphs != 0);
		return blaster.doCall(PG, cc, cost, forbid, subgraphs, crossingNumber);
	}

public:
	friend class DeletingTop10Heap<OptimalSimultaneousCrossingMinimizer::KuratowskiConstraint,double,Compare_Equals<OptimalSimultaneousCrossingMinimizer::KuratowskiConstraint> >;

	OptimalSimultaneousCrossingMinimizer() : blaster() {}
...
#endif // OGDF_ABACUS_OPTIMAL_SIMULTANEOUS_CROSSING_MINIMIZER_SUPER_PRICE_H

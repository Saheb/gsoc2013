 ...
 * last checkin:
 *   $Author: gutwenger $
 *   $Date: 2012-07-12 04:32:21 +0530 (Thu, 12 Jul 2012) $
 ***************************************************************/
...
private:
	...
	int  pairUnchecked(int e1, int e2) {
		return (e1 == e2) ? DOESNT_EXIST : ( isOrdered(e1,e2) ? pairOrdered(e1,e2) : pairOrdered(e2,e1) );
	}
	...
	int  ensureVariable(int pair) {
		if(pair == DOESNT_EXIST)
			return DOESNT_EXIST;
		int v = pair2ilp[pair];

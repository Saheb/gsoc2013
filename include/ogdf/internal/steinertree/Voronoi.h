...
protected:
	...
	void computeVoronoiRegions(const EdgeWeightedGraph<T> &wG, const List<node> &seeds);

public:
	//! Build data structure to query Voronoi regions of graph wG with given Voronoi seeds
	//! @param wG edge-weighted graph
	//! @param seeds a list of Voronoi seed nodes, the centers of the Voronoi regions
	Voronoi(const EdgeWeightedGraph<T> &wG, const List<node> &seeds)
	 : m_predecessor(wG)
	 , m_distance(wG)
	 , m_seedOfNode(wG)
	 , m_nodeList()
	{
		computeVoronoiRegions(wG, seeds);
	}
...
template<typename T>
void Voronoi<T>::computeVoronoiRegions(const EdgeWeightedGraph<T> &wG, const List<node> &seeds)
{
	Dijkstra<T> sssp;
	sssp.call(wG, wG.edgeWeights(), seeds, m_predecessor, m_distance);

	// extract Voronoi seeds for each node and Voronoi regions for each seed
	NodeArray<bool> processed(wG, false);

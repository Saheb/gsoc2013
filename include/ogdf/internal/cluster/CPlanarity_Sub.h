 ...
 * last checkin:
 *   $Author: gutwenger $
 *   $Date: 2013-04-18 17:47:14 +0530 (Thu, 18 Apr 2013) $
 ***************************************************************/
...
private:
	...
	CPlanarityMaster* master() { return (CPlanarityMaster*)master_; }
	...
	{
		return addPoolCons(cons, ((CPlanarityMaster*)master_)->getCutConnPool());
	}
	//! Adds the given constraints to the planarity cut pool
	inline int addKuraCons(ArrayBuffer<abacus::Constraint *> cons)
	{
		return addPoolCons(cons, ((CPlanarityMaster*)master_)->getCutKuraPool());
	}

	//tries to regenerate connectivity cuts
	inline int separateConnPool(double minViolation)
	{
		return separateCutPool(((CPlanarityMaster*)master_)->getCutConnPool(),minViolation);
	}
	//tries to regenerate kuratowski cuts
	inline int separateKuraPool(double minViolation)
	{
		return separateCutPool(((CPlanarityMaster*)master_)->getCutKuraPool(),minViolation);
	}
